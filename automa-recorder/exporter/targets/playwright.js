...
function generatePlaywright(ir) {
  const steps = Array.isArray(ir?.steps) ? ir.steps : [];
  const support = analyzeIrSupport({ steps }, 'playwright');
  const code = core.generatePlaywrightScript(ir);
  ...
  const insertIndex = lines.findIndex((line) => line.includes('await page.goto('));
